...
By default, this template **does not bake model weights into the Docker image**. Both model dependencies (`opendatalab/MinerU2.5-Pro-2605-1.2B` for the VLM backend and `opendatalab/PDF-Extract-Kit-1.0` for the pipeline backend) are downloaded into `/root/.cache/huggingface/` on the first parse request. This keeps the image small so RunPod Hub's build/test pipeline can finish in time.

The first request on a cold worker is therefore slow (model download + load). Two ways to fix that:

1. **Network volume for the HF cache** — mount a volume at `/root/.cache/huggingface`. The first worker downloads the weights, and every subsequent worker (warm or cold) reuses them. This is the simplest persistent-cache option.
2. **Bake models back into a private image** — fork the Dockerfile and re-add the `snapshot_download` steps. The image grows to ~22 GB and may fail RunPod Hub's test window; deploy it via **Deploy from image** instead of the GitHub repo flow.

RunPod's **Cached Models** dashboard feature only supports one model per endpoint, and MinerU needs two, so that feature is not used by this template.
...
- **You only care about cold-start latency and are baking models into a private image.** In the default template, a network volume at `/root/.cache/huggingface` is the recommended way to avoid re-downloading weights on every cold start.
