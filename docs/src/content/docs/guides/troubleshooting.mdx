...
| `model_dir` | Filesystem path of the snapshot that actually loaded. Models are cached under `/root/.cache/huggingface/`; if this is `null` on a successful job, the worker has not yet downloaded models (expected on the very first request of a cold worker)
...
**Cause**: RunPod's build pipeline has a hard 30-minute ceiling. Baking ~4 GB of model weights (MinerU VLM + PDF-Extract-Kit) into the image on top of the vLLM base can blow past the cap on a slow build-region day.

**Fix**: this template keeps models out of the image by default; they download at runtime into the HF cache. If you forked the repo and re-added model-baking `snapshot_download` steps:

- Re-trigger the build (often a transient HF-egress slowdown)
- Use `hf-xet` with `HF_XET_HIGH_PERFORMANCE=1` for faster downloads
- Split the two model downloads into separate `RUN` layers so a partial cache survives retries
- As a last resort, remove the baked models and rely on runtime downloads or a network volume at `/root/.cache/huggingface`
